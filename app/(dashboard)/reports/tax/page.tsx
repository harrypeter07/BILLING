...
export default function TaxReportPage() {
  ...
  const fetchTaxData = async () => {
    try {
      ...
      const cgst = invoices?.reduce((sum, inv) => sum + Number(inv.cgst_amount), 0) || 0
      const sgst = invoices?.reduce((sum, inv) => sum + Number(inv.sgst_amount), 0) || 0
      const igst = invoices?.reduce((sum, inv) => sum + Number(inv.igst_amount), 0) || 0
      ...
      setTaxData({
        ...
        nonGstInvoices: invoices?.filter((inv) => !inv.is_gst_invoice).length || 0,
      })
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={exportTaxReport} variant="outline" className="gap-2 bg-transparent">
