...
export default function BusinessSettingsPage() {
	...
	const handleInvoiceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		setInvoiceSettings((prev) => ({
			...prev,
			[name]:
				name === "next_invoice_number" || name === "default_due_days"
		...
		if (checked) {
			...
			if (missingFields.length > 0) {
				toast({
					...
					description: `Please set the following before enabling B2B mode: ${missingFields.join(
						", "
					)}`,
					variant: "destructive",
				});
		...
		try {
			...
			reader.onloadend = async () => {
				...
				try {
					...
					console.warn(
						"[BusinessSettings] Supabase storage unavailable, using base64:",
						storageError
					);
	...
	const saveBusinessSettings = async () => {
		...
		try {
			...
			// Prepare data for update (trim strings, convert empty to null)
			const updateData = {
				business_name: businessData.business_name?.trim() || null,
				business_gstin: businessData.business_gstin?.trim() || null,
				business_phone: businessData.business_phone?.trim() || null,
				business_email: businessData.business_email?.trim() || null,
				business_address: businessData.business_address?.trim() || null,
				logo_url: businessData.logo_url?.trim() || null,
			};
			...
			console.error("[BusinessSettings] Save failed:", error);
			toast({
				title: "Error",
	...
	const saveInvoiceSettings = async () => {
		...
		try {
			...
			const updateData = {
				invoice_prefix: invoiceSettings.invoice_prefix?.trim() || "INV",
			...
			if (error) {
				console.error("[BusinessSettings] Invoice settings save error:", error);
				throw error;
			}
			...
			console.error("[BusinessSettings] Invoice settings save failed:", error);
			toast({
				title: "Error",
				description: error?.message || "Failed to update settings",
				variant: "destructive",
	...
	return (
		<div className="space-y-6">
			...
			<Card>
				...
				{/* Right Column: Invoice Settings and B2B */}
				<div className="space-y-6">
			{/* Invoice Settings */}
			<Card>
				<CardHeader>
						...
						<>
							...
							<div>
								<Label htmlFor="business_email">
									Business Email (for B2B invoices)
								</Label>
								<Input
									id="business_email"
									name="business_email"
