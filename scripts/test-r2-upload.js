...
async function testR2Upload() {
  ...
  const envVars = {
    ...
    'R2_SECRET_ACCESS_KEY': R2_SECRET_ACCESS_KEY ? '***' + R2_SECRET_ACCESS_KEY.slice(-4) : undefined,
  ...
  console.log(`  Object key will be: invoices/${TEST_ADMIN_ID}/${TEST_INVOICE_NUMBER}.pdf`);
  ...
  try {
    ...
    if (result.success) {
      ...
      try {
        ...
        if (urlResponse.ok) {
          console.log(`  ✅ Public URL is accessible (Status: ${urlResponse.status})`);
          ...
          console.log(`  ⚠️  Public URL returned status: ${urlResponse.status}`);
